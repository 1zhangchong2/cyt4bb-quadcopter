...
#if DEBUG_UART_USE_INTERRUPT             // 如果启用了 debug 串口中断
        debug_interrupr_handler();       // 调用 debug 串口接收数据处理函数 数据会被 debug 串口函数正确读取
#endif                                   // 如果修改了 DEBUG_UART_INDEX 并且没有启用 则需要放到对应的串口中断去
...
void uart4_isr (void)
{
    if(uart_isr_mask(UART_4))            // 串口4接收中断
    {
        ...
        uart_receiver_handler();                                                                // 用于接收回调的函数
